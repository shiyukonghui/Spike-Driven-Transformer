...
impl MemoryPage {
    /// PATCH (rust-sdt 排查)：收集本页被钉住切片（descriptor 强引用 >= 2）的分配 id
    pub fn pinned_ids(&self) -> Vec<u64> {
        let mut out = Vec::new();
        for slice in self.slices.iter() {
            if slice.handle.strong_count() >= 2 {
                out.push(slice.handle.descriptor().id.value as u64);
            }
        }
        out
    }

    /// PATCH (rust-sdt 排查)：统计本页各切片 descriptor 的强引用计数直方图。
    /// 返回 [count_1, count_2, count_3, count_4plus]。
    ///
    /// count=1：仅页自身持有（free，可复用）；
    /// count>=2：还有外部克隆（HandleContainer / Binding 等）。
    pub fn strong_count_histogram(&self) -> [u64; 4] {
        let mut hist = [0u64; 4];
        for slice in self.slices.iter() {
            let count = slice.handle.strong_count();
            let idx = match count {
                1 => 0,
                2 => 1,
                3 => 2,
