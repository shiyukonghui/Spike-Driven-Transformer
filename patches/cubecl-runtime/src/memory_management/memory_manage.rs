...
impl<Storage: ComputeStorage> MemoryManagement<Storage> {
    ...
    pub fn reserve(&mut self, size: u64) -> Result<ManagedMemoryHandle, IoError> {
        ...
        // PATCH (rust-sdt 排查)：记录 分配id -> 请求大小，供池诊断反查被钉住切片的来源大小
        let reserved = self.reserve_inner(size)?;
        crate::alloc_sizes::record(reserved.descriptor().id.value as u64, size);
        Ok(reserved)
    }
    ...
    pub fn bind(
        ...
        // PATCH (rust-sdt 排查)：client.empty() 句柄（assigned）不经 reserve 创建，
        // 其真实大小 = reserved 切片的请求大小；登记映射供池诊断反查钉住张量大小。
        if let Some(sz) = crate::alloc_sizes::lookup(descriptor.id.value as u64) {
            crate::alloc_sizes::record(assigned.descriptor().id.value as u64, sz);
        }
