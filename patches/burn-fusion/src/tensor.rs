...
// PATCH (rust-sdt 显存排查)：DropOp 执行统计（总数 + 8MB 桶）。
// 若 8MB 激活张量的 drop 执行数远小于其分配数（见 cubecl 池新页数），
// 说明这些张量从未走 DropOp（ReadOnly/SharedTensors 路径）→ 句柄泄漏。
pub(crate) static DROP_TOTAL: std::sync::atomic::AtomicU64 = std::sync::atomic::AtomicU64::new(0);
pub(crate) static DROP_8MB: std::sync::atomic::AtomicU64 = std::sync::atomic::AtomicU64::new(0);

impl<RO: FusionRuntime> Operation<RO> for DropOp {
    fn execute(&self, handles: &mut burn_ir::HandleContainer<RO::FusionHandle>) {
        match handles.remove_handle(self.id) {
            Some(burn_ir::Handle::Existing(_h)) => {
                DROP_TOTAL.fetch_add(1, Ordering::Relaxed);
                if self.size_bytes == 8_388_608 {
                    DROP_8MB.fetch_add(1, Ordering::Relaxed);
                }
            }
            other => {
                // NotInit / None：与原实现一致（remove_handle 对缺失 id 无害）
                core::mem::drop(other);
            }
        }
    }
    ...
    fn drop(&mut self) {
        ...
        match self.status(count) {
            TensorStatus::ReadWrite => {
                ...
                // PATCH：计算张量字节数（shape_out 是 swap 后的空壳，这里用 elem_count 推回）
                let size_bytes = {
                    let elem = self.dtype.size() as u64;
                    shape.iter().map(|d| *d as u64).product::<u64>() * elem
                };
